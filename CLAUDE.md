...
## Current state: Milestone 46 — multi-block selection in the editor canvas
...
position) — it never enters `export_script()`, persistence, or RUN. It lives entirely in
[`BlockCanvas`](scripts/block_canvas.gd); [`BlockView`](scripts/block_view.gd) is untouched (the
selection outline is applied by duplicating each selected panel's own stylebox).

### Selection — a set of statement-block dicts, tracked by identity

[`BlockCanvas._selected`](scripts/block_canvas.gd) holds references to the selected **statement** block
dicts (identity via `is_same`). Because the block dicts in `_stacks` persist across a `_render()` (only
the Control tree is rebuilt), a rendered panel's `blk_array[blk_index]` resolves back to the same dict —
so a selection **survives a re-render**. Reporter pills are deliberately *not* selectable: double-click's
"block + everything after it in this script" and the rubber-band are statement-level gestures.
[`_apply_selection_highlight`](scripts/block_canvas.gd) (called at the end of `_render`) outlines each
selected panel by duplicating its category stylebox and adding a white border — no `BlockView` change.
...
- **Click** a statement block → select it alone; **Shift/Cmd-click** → toggle it in/out
  ([`_toggle_selected`](scripts/block_canvas.gd)).
- **Double-click** a block → select it **and all following siblings at that body level**
  (`array.slice(index)`) — the "block + everything after it in this script" gesture (additive with a
  modifier, else replacing the selection).
...
edit since M16); selecting **reporter pills** (selection is statement-level); a **copy/paste across
sprites** clipboard (duplicate lands in the same sprite); and a **marquee-vs-C-block** nicety (a
rubber-band over a `forever` highlights both it and the inner blocks it covers — the topmost filter
collapses them for any operation, but both still draw outlined).
...
3. Press **F5** (Run Project). The main scene is now `editor.tscn`: the **block
   ...
   **Select multiple blocks** (M46): **click** a block to select it (a white outline);
   **Shift/Cmd-click** to add or remove blocks one at a time; **double-click** a block to select it
   **and every block after it in that script**; or **drag a box over empty canvas** (rubber-band) to
   select everything it covers (Shift-drag adds to the current selection). Click empty canvas to clear.
   With a selection: **drag** any selected block to **move them all together** (they gather into one
   run, snapping into a stack or landing free — drag them onto the palette to delete the lot), press
   **Delete/Backspace** to delete the selection, or **Cmd/Ctrl+D** to **duplicate** it as a new stack.
   (Selection is editor-only and not saved; switching sprites clears it. There's no undo.)
...
Shift-click / double-click / rubber-band to select statement blocks in the canvas, then delete /
move-together / duplicate them). Selection is editor-only UI state in [`BlockCanvas`](scripts/block_canvas.gd),
never serialized; no block-data-shape change, no interpreter/runtime change, and [`BlockView`](scripts/block_view.gd)
is untouched. The notes below are kept as written for earlier milestones.
