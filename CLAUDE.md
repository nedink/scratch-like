...
it a **`ColorPickerButton`** ([`BlockPalette._add_group_header`](scripts/block_palette.gd)) — the same
picker idiom the inspector and the Paint view already use. The button's **fill is the category colour**
(a Scratch-like coloured title bar), and the title text is drawn black or white by luminance
([`_style_header_text`](scripts/block_palette.gd)) so it stays legible on light and dark categories
alike. It carries no `palette_opcode` meta, so [`_chip_at`](scripts/block_palette.gd) skips it and the
palette's `_input` drag machinery never starts a drag from it — GUI handles the click, exactly like the
Make-a-Variable button. Clicking any title works, including **MY BLOCKS** (whose category is `custom`)
and **LISTS**.
...
- **`color_changed` (live, as you drag)** → [`_on_category_color_changed`](scripts/block_palette.gd):
  ...
  writes the `BlockStyles` property `_styles` reads), re-contrast the header text, and
  [`_canvas.refresh()`](scripts/block_canvas.gd) so the canvas blocks recolour **immediately** (the
  header tracks its own fill; the picker stays open). It deliberately does **not** rebuild the palette
  here — that would free the open `ColorPickerButton`.
- **`popup_closed` (once, when the picker closes)** →
...
scripts/
  ...
  block_palette.gd         Block palette (M11): lists opcodes as chips (reporters too, as pills — M14); on drag, mints a fresh block and hands it to the canvas; each group title is a ColorPickerButton (M49, _add_group_header) — clicking it recolours that category: _on_category_color_changed sets BlockView.set_category_color + refreshes the canvas live as you drag, and _on_category_color_committed saves blocks/block_styles.tres + rebuilds the chips when the picker closes; rebuild() re-renders the chips when the editor re-scopes the variable dropdowns on a sprite switch (M19); draws a "Make a Variable" button atop the variables group (M20) and, beneath it, a Rename/Delete MenuButton row per in-scope variable (M21), all calling back to the editor; draws a "My Blocks" group (M30) — a "Make a Block" button plus one pre-named call chip per custom block the sprite defines (BlockView.project_custom_blocks); define/call carry palette:false so palette_groups skips them, so this is the only place they enter the palette; a call chip carries one args slot per the block's declared parameters (M31, project_custom_block_params), the drag re-minting the args dict from the chip's palette_params
