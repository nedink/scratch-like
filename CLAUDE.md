...
## Current state: Milestone 51 — keyboard authoring mode (cursor + fuzzy block picker)

**Goal of this milestone:** make the block editor **fully keyboard-drivable for fast scripting, alongside**
the existing click/drag — pick at will. The user's first framing ("click a label, retype the block, arrows
between statements, Return for a new block") sounds like inline text parsing, but the chosen model is a
**cursor + fuzzy block picker**, not a natural-language parser: a keyboard cursor navigates **statement
gaps** and **input slots**, and a fuzzy picker (filtering opcodes by their natural-language label) inserts a
block at the cursor. Nested reporter expressions (`move (score + 1)`, `if (touching Ball?)`) are built
**recursively** — pick `+` into a slot, the cursor descends into its operands, fill each (possibly with more
reporters) — so the **full** language is reachable with no parser. Like every editor milestone since M27 it
is a **pure editor-side interaction over the data-owned block model**: **no new opcode, no
block-data-shape change, no runtime change, no `editor.tscn` change.** The cursor is editor-only UI state
(the sibling of selection M46 / collapse M47) — it never enters `export_script()`, persistence, or RUN.

### The cursor — a data-referenced position, the sibling of selection

[`BlockCanvas._cursor`](scripts/block_canvas.gd) is a tagged dict: `{kind:"gap", array, index}` (an
insertion point in a live body Array, `0..size`), `{kind:"slot", inputs, key}` (an input slot), or
`{kind:"new", pos}` ("start a new top-level stack here"), or `{}` (none). Like `_selected` it is stored as
**data references**, never Control references, so it **survives `_render()`** (which frees and rebuilds the
whole Control tree): it is re-resolved to a widget each render by matching the same meta BlockView stamps —
`body_array`+index for a gap ([`_gap_global_marker`](scripts/block_canvas.gd)), `slot_inputs`+`slot_key` for
a slot ([`_cursor_control`](scripts/block_canvas.gd)). A gap/slot cursor whose data no longer renders (its
...
[`_handle_cursor_nav`](scripts/block_canvas.gd) (a new branch of [`_handle_key`](scripts/block_canvas.gd),
run **before** the M46/M47 selection ops and **independent of `_selected`**, so keys work whenever a cursor
is placed): **Up/Down** step [`_ordered_gaps`](scripts/block_canvas.gd) — a *data* walk of `_stacks` (each
`blocks` array, recursing into C-block/hat bodies, skipping collapsed stacks) giving stable document order;
from a slot they first ascend to the gap before the containing statement
([`_find_owner_statement`](scripts/block_canvas.gd) via [`_inputs_reach`](scripts/block_canvas.gd)).
...
**Enter** or **typing** opens [`_open_picker`](scripts/block_canvas.gd) — a single
`PopupPanel` + search `LineEdit` + results `ItemList`, built once by
[`_ensure_picker`](scripts/block_canvas.gd) (the lazy-popup pattern of `BlockPalette._ensure_color_popup`)
and positioned just below the caret. [`_picker_candidates`](scripts/block_canvas.gd) scopes the list to the
cursor: a **gap** → statements/C-blocks (hats only at a `"new"` stack, never mid-stack — consistent with
`_nearest_gap` forbidding hat snaps); a **slot** → reporters whose `reporter_output_type` matches the slot's
`slot_type` (the **M23 boolean-vs-value rule**, now for keyboard insert). `palette:false` opcodes
(`define`/`call`/`param`) are excluded (drawn from `palette_groups`).
...
its body** if it's a hat/C-block, else **into its first header slot** if it has one
([`_first_slot_key`](scripts/block_canvas.gd) — so `go to x: y:` lands on the `x` slot ready to fill, the
statement twin of a reporter descending into its first operand), else (a slotless statement) moves to the
...
## Earlier: Milestone 50 — category recolours are per-project, not a global edit

**Goal of this milestone:** make a category recolour (M49) **save with the project**, not as a permanent
global change. M49 wrote a recolour into [`blocks/block_styles.tres`](blocks/block_styles.tres), so it was
shared across every project and stuck until reverted in git. M50 moves the override into the **project
`.json`**: a recolour now rides with the project that made it (it loads when that project opens, resets on
NEW), and **any category the project doesn't recolour draws its default**. It is a small editor-side
follow-on — **no new opcode, no block-data-shape change, no runtime change, no `editor.tscn` change** — that
changes *where the override is stored and read from*, not the picker UI.

### The override becomes a project property

The editor gains a `_block_colors` model ([`editor.gd`](scripts/editor.gd)) — `category name -> hex string`
— the colour sibling of `_background` / `_grid_settings`: seeded **empty** by
[`_seed_demo`](scripts/editor.gd) (so the stock demo recolours nothing → every block draws its default),
serialised under a top-level **`block_colors`** key
([`_serialize_project`](scripts/editor.gd) / [`_apply_project`](scripts/editor.gd) — optional, so a pre-M50
save opens with no overrides), and stashed across the RUN→ESC round trip
([`_restore_block_colors`](scripts/editor.gd)). On every project load it's pushed into the new static
[`BlockView.project_block_colors`](scripts/block_view.gd) (`category -> Color`) by
[`_sync_block_colors`](scripts/editor.gd) — the colour twin of `_sync_background`, run before `_show`, so
the palette rebuild / canvas render draw each category in its override or (where the project carries none)
its default. `project_block_colors` is the static-as-project-model pattern of `project_variables` /
`project_sprites`.

### Reading and writing the colour

[`BlockView.category_color`](scripts/block_view.gd) now reads the project override
(`project_block_colors`) first, else [`default_category_color`](scripts/block_view.gd) — which is the M48
`BlockStyles` resource value, else the hardcoded `_CATEGORY_COLORS` (the stock Scratch hue). So `BlockStyles`
([`blocks/block_styles.tres`](blocks/block_styles.tres)) stays the **inspector-editable default** provider
(M48 preserved), and the in-app picker writes only the project. M49's `BlockView.set_category_color` /
`save_styles` (the `.tres` writers) are **removed**; the palette instead calls back to the editor, like the
Make-a-Variable hooks: [`BlockPalette._on_recolor_category`](scripts/block_palette.gd) →
[`editor._recolor_category`](scripts/editor.gd) (live, as the picker drags: write `_block_colors` +
`project_block_colors`, `_canvas.refresh()`) and `_on_reset_category` →
[`editor._reset_category_color`](scripts/editor.gd) (the "Reset to default" button now **clears** the
override, so the category falls back to its default and the project drops the entry). The picker close just
rebuilds the chips ([`_on_category_color_committed`](scripts/block_palette.gd)) — persistence already
happened live, so there's no `.tres` write. The working `.tres` was reverted to stock (M49 testing had baked
`control`/`motion` overrides into it — exactly the permanent global change M50 retires).

What this leaves deferred: the same M49 deferrals (**reset-all-categories** one-shot — reset is per-category;
**renaming/adding categories** — the set is fixed by `BlockStyles`' exports; and the standing M48 deferral, a
shared editor **`Theme`** for label/font styling). Editing the **defaults** is still the M48 inspector edit of
`block_styles.tres` (a deliberate global, unlike a per-project recolour).

---

## Earlier: Milestone 49 — recolour a category from its palette header

**Goal of this milestone:** let the user **change a block category's colour from the editor UI** — click a
section title in the palette and pick a new colour — instead of hand-editing
[`blocks/block_styles.tres`](blocks/block_styles.tres) in the Godot inspector (the only way M48 left).
It is a small editor-side follow-on to M48's data-keyed colour: **no new opcode, no block-data-shape
change, no runtime change, no `editor.tscn`/`editor.gd` change** — the whole feature lives in
[`BlockPalette`](scripts/block_palette.gd) plus a two-method colour-store API on
[`BlockView`](scripts/block_view.gd). It builds directly on M48: the per-category colour already comes
from the editable `BlockStyles` resource via [`category_color`](scripts/block_view.gd), and `_tint`
already applies it by duplicating each block shell's stylebox — M49 just makes that resource editable
*in-app* and persists it.

### The header becomes a colour picker

Through M48 each palette group title was a mouse-ignored `Label` (colour-fonted, transparent). M49 makes
it a **flat `Button`** ([`BlockPalette._add_group_header`](scripts/block_palette.gd)) that **keeps the
exact M48 look** — the title in the category's colour on a transparent background, held across
hover/pressed/focus by [`_style_header_text`](scripts/block_palette.gd) so a flat button's default skin
never overrides it. (A `ColorPickerButton` was the first attempt but is wrong here: it paints its colour
swatch over the *whole* button, hiding the title text — so the click pops a **separate `ColorPicker`**
instead.) The header carries no `palette_opcode` meta, so [`_chip_at`](scripts/block_palette.gd) skips it
and the palette's `_input` drag machinery never starts a drag from it — GUI handles the click, exactly
like the Make-a-Variable button. Clicking any title works, including **MY BLOCKS** (whose category is
`custom`) and **LISTS**.

### One shared picker, lazily built and reused

A header press ([`_on_header_pressed`](scripts/block_palette.gd)) opens a **single shared `ColorPicker`
in a `PopupPanel`** ([`_ensure_color_popup`](scripts/block_palette.gd) builds it on first use and reuses
it), pre-set to the category's current colour and `popup_centered()`. The popup is added as a palette
child but **skipped by `rebuild()`**, so a live recolour can rebuild the chips without freeing the open
popup. The category + header being edited are held in `_recolor_category` / `_recolor_header` so the
picker's argument-less signals know what they're acting on.

### Live recolour, then persist on close

The picker's signals drive two phases, kept apart on purpose so a live recolour never disturbs the open
picker:

- **`color_changed` (live, as you drag)** → [`_on_category_color_changed`](scripts/block_palette.gd):
  set the colour **in memory** via the new [`BlockView.set_category_color`](scripts/block_view.gd) (which
  writes the `BlockStyles` property `category_color` reads), recolour the header title to match, and
  [`_canvas.refresh()`](scripts/block_canvas.gd) so the canvas blocks recolour **immediately**. It
  deliberately does **not** rebuild the palette here — that would free the open popup.
- **`popup_hide` (once, when the picker closes)** →
  [`_on_category_color_committed`](scripts/block_palette.gd): persist the resource to disk with the new
  [`BlockView.save_styles`](scripts/block_view.gd) (`ResourceSaver.save` back to
  `blocks/block_styles.tres` — one write), then **deferred** `rebuild()` so every *chip* in that category
  re-tints to the final colour. Deferred because `rebuild()` frees the palette's children and we're
  inside the button's own signal.

Persistence is to the **`.tres`, not the project `.json`** — colours are a global editor/styling concern
(the M48 store), not per-project data, so a recolour sticks across relaunches and across projects, and
shows up as a modified `blocks/block_styles.tres` in git (commit it to keep the palette, or revert to
...
## Earlier: Milestone 48 — block rendering via editable Godot scenes

**Goal of this milestone:** stop hand-building block visuals in code so the user can **style blocks
themselves in the Godot editor**. Through M47, [`BlockView`](scripts/block_view.gd) `new()`-ed every
`Control` and applied all styling in code (`_box()` styleboxes — per-category colour, corner radii,
padding — plus label font colours and the literal-field/dropdown look). M48 moves the **shape and look
into editable `.tscn` shells** under [`blocks/`](blocks), one per visual shape, that the renderer
**instantiates and populates**. It is a pure **rendering refactor** — **no opcode, no block-data-shape
change, no runtime change, and no canvas/palette change** — because both consumers
([`BlockCanvas`](scripts/block_canvas.gd) / [`BlockPalette`](scripts/block_palette.gd)) walk the rendered
tree **generically by meta** (`blk_array`, `blk_index`, `slot_inputs`, `slot_key`, `slot_type`,
`lit_key`, …), which `BlockView` still stamps on the same logical nodes. They never cared whether a node
was `new()`-ed or instantiated from a scene.

### What moved into scenes, what stayed in code

The split is **shape/look in scenes, dynamic assembly + the one data-keyed colour in code**:

- **Scenes own the shell.** Eight scenes in [`blocks/`](blocks), each a small `Control` tree whose
  stylebox/fonts/layout you restyle in the editor: [`statement_block.tscn`](blocks/statement_block.tscn)
  (also hats and C-blocks — a `PanelContainer` exposing a `%Content` VBox where the renderer adds the
  header and, for hats/C-blocks, the body stack), [`reporter_pill.tscn`](blocks/reporter_pill.tscn) /
  [`boolean_pill.tscn`](blocks/boolean_pill.tscn) (the round value pill vs the angular boolean pill —
  now divergeable toward a real hex look), [`number_field.tscn`](blocks/number_field.tscn) /
  [`text_field.tscn`](blocks/text_field.tscn) (the oval vs rectangular literal `LineEdit`s),
  [`enum_field.tscn`](blocks/enum_field.tscn) (the dropdown `OptionButton`),
  [`empty_slot.tscn`](blocks/empty_slot.tscn) (the faint empty-C-body drop zone), and
  [`collapsed_bar.tscn`](blocks/collapsed_bar.tscn) (the M47 summary bar — a `PanelContainer` exposing a
  `%Row` HBox the renderer fills with the summary label + `+N` badge).
- **The renderer keeps all the dynamic assembly.** `build_block` / `build_reporter` /
  `build_collapsed_stack` / `_param_pill` / `_literal_field` / `_enum_field` / `_empty_slot` now
  `instantiate()` a shell and populate it (header via the unchanged `_header_from_template`, fields via
  `.text` / `add_item`), instead of `new()`-ing Controls. `build_stack` / `build_input` / the cascade
  walkers / coercion are untouched. The dead `_box()` / `_BLOCK_PAD` are gone.
- **Per-category colour stays data-applied.** A block's colour is keyed by its **category** (data), so it
  can't live in a single shared scene. It comes from an **editable resource** — `BlockStyles`
  ([`scripts/block_styles.gd`](scripts/block_styles.gd), one exported `Color` per category) loaded from
  [`blocks/block_styles.tres`](blocks/block_styles.tres) and editable as colour swatches in the inspector.
  [`category_color`](scripts/block_view.gd) reads it (falling back to the kept hardcoded `_CATEGORY_COLORS`
  if the resource is missing — a non-editor caller), and the new [`_tint`](scripts/block_view.gd)
  **duplicates a shell's `panel` stylebox and overrides only `bg_color`** — so the user's *shape* edits
  survive and only the data-keyed colour is applied. Duplicating (rather than mutating the shared scene
  stylebox) also keeps the canvas's selection-outline working, which itself duplicates that stylebox.

### What this leaves deferred

**Label font/colour** is still applied in code (white, via `_push_label`) rather than a Theme — it reads
on every category colour, so it waited; a shared editor `Theme` for label/font styling is a follow-on.
The **collapse chevron** (M47) and the marquee/highlight overlays are canvas chrome, not block visuals, so
they stay code-built. **True hexagon/notch geometry** (the long-standing deferral) is now *reachable* —
swap a pill scene's `StyleBoxFlat` for a textured/`NinePatch` look — but isn't done here. And per the
project's testing reality, this milestone is **not F5-verified** (Claude can't run Godot, and `.tscn`/
`.tres` parsing can't be checked outside the editor); the scenes are hand-authored in format-3.

---

## Earlier: Milestone 47 — collapsable scripts in the editor canvas
...
### Rendering — a one-line summary bar, and a collapse chevron in the header
...
A stack with **more than one block** (`count_blocks > 1` — something to hide) gets a **collapse chevron**
inside its first block's header ([`_add_collapse_chevron`](scripts/block_canvas.gd) →
[`_first_header`](scripts/block_canvas.gd), which finds the header `HBoxContainer` — the collapsed bar's
`%Row`, or an expanded block's header under `%Content`). The glyph is plain ASCII — **`-` expanded /
`+` collapsed** — so it renders in the web export (the geometric-triangle Unicode `▼`/`▶` tofus there);
an **`SIZE_EXPAND_FILL` spacer** before it pushes it to the header's right edge, so on a hat/C-block
(whose panel is as wide as its body) it right-justifies the full block width away from the text. The
chevron is stamped with its owning stack dict as the `collapse_stack` meta. There is **no left gutter**
(an earlier cut wrapped each stack in one), so a free-landing dragged stack stores its `pos` straight
from the ghost ([`_land_pos`](scripts/block_canvas.gd)).
...
- **Click the chevron.** The press handler ([`_input`](scripts/block_canvas.gd)) checks
  [`_collapse_toggle_at`](scripts/block_canvas.gd) **before** `_front_stack_at` (the chevron sits inside
  the first block's header, so checking it first gives a press on it priority over grabbing the block it
  lives in — it's found by its global rect via the `collapse_stack` meta, so its `MOUSE_FILTER_IGNORE`
  doesn't matter): a hit flips that stack's `collapsed` and re-renders, never selecting / dragging /
  clearing.
...
3. Press **F5** (Run Project). The main scene is now `editor.tscn`: the **block
   ...
   **Collapse a script** (M47): click the **`-` chevron** at the right of any multi-block script's first
   block to fold it into a one-line summary bar (the first block's label + a `+N` count of the blocks
   hidden), and the **`+` chevron** to expand it again — or select some blocks and press **Cmd/Ctrl+E** to
   collapse/expand their scripts at once. A collapsed script still drags, selects, deletes, and duplicates as one unit.
   Collapse is editor-only and not saved (a sprite switch re-expands everything), like the selection.
   **Type to script** (M51) — the block editor is fully keyboard-drivable alongside drag/drop. **Click**
   ...
   **inside** it, so the next block nests; picking a reporter into a slot (e.g. `+` into `move`'s steps)
   ...
   slot. (Keyboard authoring is editor-only and edits the same blocks as drag/drop — RUN/SAVE carry them.)
   ...
   **Recolour a block category** (M49) by clicking its **section title** in the palette (the coloured
   ...
   blocks in that category recolour live as you pick. The choice is **saved with the project** (M50) — it
   reloads when you OPEN that project, **NEW resets every category to its default**, and any category you
   didn't recolour draws its default. **Reset to default colour** (the button under the picker) returns a
   category to its stock hue and drops the override from the project. (The *defaults* themselves are still
   editable globally by opening `blocks/block_styles.tres` in the Godot inspector — M48.)
...
**M51 added no opcodes** — it is a pure **editor interaction** change (a keyboard authoring mode: a cursor
navigates statement gaps / input slots, and a fuzzy block picker inserts blocks at the cursor, building
nested reporter expressions recursively). The cursor is editor-only UI state in
[`BlockCanvas`](scripts/block_canvas.gd) (never serialized); the only `block_view.gd` additions are two
match-only helpers ([`opcode_label`](scripts/block_view.gd) / [`opcode_template`](scripts/block_view.gd)).
No `_OPCODES` change, no block-data-shape change, no interpreter/runtime change, no `editor.tscn` change.
The notes below are kept as written for earlier milestones.

**M50 added no opcodes** — it is a pure **editor persistence** change (a category recolour is stored in the
project `.json` under a `block_colors` key instead of the global `blocks/block_styles.tres`, so it loads with
the project and resets on NEW; missing categories use their default). No `_OPCODES` change, no
block-data-shape change, no interpreter/runtime change, no `editor.tscn` change — the editor owns a
`_block_colors` model pushed to the new static [`BlockView.project_block_colors`](scripts/block_view.gd) per
project, and the palette recolour calls back to the editor (M49's `BlockView.set_category_color` /
`save_styles` are removed). The notes below are kept as written for earlier milestones.

**M49 added no opcodes** — it is a pure **editor UI** change (recolour a block category by clicking its
palette section title, which opens a colour picker that edits the category colour live; **M50 changed where
that persists** — see above). No `_OPCODES` change, no block-data-shape change, no interpreter/runtime
change. The notes below are kept as written for earlier milestones.

**M48 added no opcodes** — it is a pure **rendering refactor** (block visuals moved from code into
editable `.tscn` shells under [`blocks/`](blocks), one per shape, that [`BlockView`](scripts/block_view.gd)
instantiates and populates; per-category colour comes from the editable `BlockStyles` resource
[`blocks/block_styles.tres`](blocks/block_styles.tres)). No `_OPCODES` change, no block-data-shape change,
no interpreter/runtime change, and no canvas/palette change (they walk the rendered tree by meta, which is
stamped on the same nodes as before). The notes below are kept as written for earlier milestones.

**M47 added no opcodes** — it is a pure **editor interaction** change (collapsable scripts: collapse a
top-level stack to a one-line summary bar via a collapse chevron or Cmd/Ctrl+E, expand it the same way).
