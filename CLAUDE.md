...
## Current state: Milestone 49 — recolour a category from its palette header
...
palette section title, which opens a `ColorPickerButton` that edits the M48 `BlockStyles` colour store
live and persists it to [`blocks/block_styles.tres`](blocks/block_styles.tres) on close). No `_OPCODES`
change, no block-data-shape change, no interpreter/runtime change, no `editor.tscn`/`editor.gd` change —
it lives in [`BlockPalette`](scripts/block_palette.gd) + a two-method API on
[`BlockView`](scripts/block_view.gd) (`set_category_color` / `save_styles`). The notes below are kept as
written for earlier milestones.
