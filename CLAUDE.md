...
These three ops stay **statement-level**: [`_topmost_selected_in_order`](scripts/block_canvas.gd) walks
only the statement blocks (and their bodies), so a **selected reporter pill is ignored** by move / delete
/ duplicate (a pure-pill selection is a no-op for them — a reporter can't become a free-floating stack,
and deleting one would mean reverting its slot to a default, which only its own pickup gesture does). So
pill selection is for highlighting/visibility; the group operations remain about statements.
...
edit since M16); **group operations on reporter pills** (a pill can be *selected* and *highlighted* now,
but move/delete/duplicate stay statement-level — see above); a **copy/paste across sprites** clipboard
(duplicate lands in the same sprite); and a **marquee-vs-C-block** nicety (a rubber-band over a `forever`
highlights both it and the inner blocks it covers — the topmost filter collapses them for any operation,
but both still draw outlined).
...
3. Press **F5** (Run Project). The main scene is now `editor.tscn`: the **block
   ...
   **Select multiple blocks** (M46): **click** a block — a statement *or* a reporter pill — to select it
   ...
   With a selection: **drag** any selected statement to **move them all together** (they gather into one
   ...
   **Delete/Backspace** to delete the selection, or **Cmd/Ctrl+D** to **duplicate** it as a new stack.
   (Move/delete/duplicate act on the selected statements — a selected pill is highlighted but those ops
   skip it. Selection is editor-only and not saved; switching sprites clears it. There's no undo.)
...
Shift-click / double-click / rubber-band to select blocks in the canvas — statement blocks *and* reporter
pills, double-click selecting a block plus all of its nested children — then delete / move-together /
duplicate the selected statements). Selection is editor-only UI state in [`BlockCanvas`](scripts/block_canvas.gd),
